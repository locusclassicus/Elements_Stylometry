# in this lesson, we'll prepare our data for regression analysis
...
cicero_att <- cicero_att %>% filter(grepl("\\(\\d*?\\)", date))

cicero_att$date <- gsub(".*\\(", "", cicero_att$date)
cicero_att$date <- gsub(").*$", "", cicero_att$date)

cicero_att <- cicero_att %>% mutate(date = as.numeric(date)) %>% 
  mutate(dest = "Atticus")
...
cicero_letters <- cicero_letters %>% bind_rows(cicero_fam) %>% 
  mutate(date = date*(-1))
save(cicero_letters, file = "CiceroLetters.Rdata")
